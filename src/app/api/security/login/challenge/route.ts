...
export async function POST(request: Request) {
    ...
    if (!isCountryAllowed(settings, clientInfo.countryCode)) {
        ...
        if (settings.notify_on_suspicious && (settings.alert_email || profile?.email || context.profile.email)) {
    ...
    await sendSecurityCodeEmail({
        ...
        code: challenge.code,
        purposeLabel: 'completar el inicio de sesión',
    });

    await logSecurityEvent({
        profileId: context.profile.id,
        email: profile?.email || context.profile.email,
        eventType: 'LOGIN_CHALLENGE_SENT',
        request,
        metadata: { challengeId: challenge.challengeId, suspicious: isSuspicious },
    });
